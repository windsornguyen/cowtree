...
def main() -> int:
    ...
    plot_elapsed(data, args.out_dir / "elapsed_seconds.png")
    plot_time_ratio(data, args.out_dir / "time_ratio.png")
    plot_payload(data, args.out_dir / "payload_copied.png")
    plot_payload_avoided(data, args.out_dir / "payload_avoided.png")
    write_summary(data, args.out_dir / "summary.rst")
    print(args.out_dir)
    code = 0
    return code
...
def plot_elapsed(data: BenchmarkResults, out: Path) -> None:
    ...
    ax.bar(
        ...
        width,
        label="cowtree",
        color=method_color(Method.COWTREE),
    )
...
def plot_payload(data: BenchmarkResults, out: Path) -> None:
    ...
    ax.bar(
        [index - width / 2 for index in x],
        [mib(row.payload_copied_bytes) for row in series.git],
        width,
        label="git worktree",
        color=method_color(Method.GIT),
    )
    ax.bar(
        [index + width / 2 for index in x],
        [mib(row.payload_copied_bytes) for row in series.cowtree],
        width,
        label="cowtree",
        color=method_color(Method.COWTREE),
    )
...
def plot_payload_avoided(data: BenchmarkResults, out: Path) -> None:
    series = plot_series(data)
    avoided = [
        mib(git.payload_copied_bytes - cowtree.payload_copied_bytes)
        for git, cowtree in zip(series.git, series.cowtree, strict=True)
    ]
...
def plot_series(data: BenchmarkResults) -> PlotSeries:
    ...
    git = [find_summary(data, profile, Method.GIT) for profile in profiles]
    cowtree = [find_summary(data, profile, Method.COWTREE) for profile in profiles]
