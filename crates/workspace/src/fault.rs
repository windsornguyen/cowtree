...
pub(crate) fn checkpoint(point: &str) {
    ...
    pause(point);
    #[cfg(feature = "fault-injection")]
    if std::env::var("COWTREE_WORKSPACE_CRASH_AT").as_deref() == Ok(point) {
        std::process::exit(73);
    }
...
fn pause(point: &str) {
    ...
    if std::env::var("COWTREE_WORKSPACE_PAUSE_AT").as_deref() != Ok(point) {
        return;
    }
    let Some(path) = std::env::var_os("COWTREE_WORKSPACE_PAUSE_FILE") else {
        std::process::exit(74);
    };
...
pub(crate) fn io_error(point: &str) -> std::io::Result<()> {
    #[cfg(feature = "fault-injection")]
    if std::env::var("COWTREE_WORKSPACE_IO_ERROR_AT").as_deref() == Ok(point) {
