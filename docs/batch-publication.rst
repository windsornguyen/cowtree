...
``Batches(workspace)`` publishes disjoint captured changes from several managed
leaves in one SQLite epoch. Capture each writer with ``Captures.capture``, then
prepare the complete membership, check its union once, and commit that exact batch.

.. code-block:: python

   batches = Batches(workspace)
...
the batch. Members cannot publish separately through ``Publications.commit``.
...
Individual receipts remain available through ``Publications.result`` within their
...
Run ``uv run pytest mounted/test_batches.py`` on a native copy-on-write filesystem.
